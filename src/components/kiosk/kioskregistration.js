...
const KioskRegistration = () => {
  ...
  const registerNewPatient = async (data) => {
    ...
    // Get the current QR result (same logic as in handleSubmit)
    ...
    // Update the cleanup section to use currentQrResult
    ...
    if (currentQrResult?.temp_id) {
      ...
      // The backend should handle the cleanup, but we can log it
